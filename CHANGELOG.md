...
### Fixed
- **`status` reports installs from older releases.** On a machine still set up by 4.x, it showed 0 commands; on one set up by v2, it said nothing was installed. `status` now lists commands left by 4.x, v3 or v2 under their own heading, by the names you typed, and says whether the next install replaces them (4.x) or they are safe to delete (v2, v3).
